...
then cached in data/auth/token.json and reused.
    ...
    python m1-voice/extract_emails.py

Output: one JSON object per line in data/raw_emails.jsonl
        {"id", "date", "to", "subject", "body"}

Pre-event note: this file is already cached in the repo, so you do NOT need to
run this live. It's here so the cohort (and you) can regenerate the corpus.
"""
